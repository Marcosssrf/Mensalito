...
export default function WhatsAppPage() {
    ...
    function handleReconnect() {
        ...
        api.post<WAStatus>('/tenants/me/whatsapp/provision')
            .then((r) => setStatus(r.data))
            .catch((e) => setProvisionError(e.response?.data?.message ?? e.message ?? 'Erro desconhecido'))
                    ...
                    {!connected && status?.qrCodeBase64 && (
                        <div className="ms-qr-panel" style={{ marginBottom: 28, padding: 24, border: '1px solid #e5e7eb', borderRadius: 12, background: '#fff', display: 'flex', alignItems: 'center', gap: 24 }}>
                            ...
                            <div>
                                ...
                                <p style={{ fontSize: 13, color: '#6b7280', margin: 0 }}>Abra o WhatsApp no celular → Dispositivos conectados → Conectar dispositivo</p>
                            </div>
                        </div>
                    )}
